...
* This enhancement updates the supported syntax for specifying the delivery mode for log forwarding in the `ClusterLogForwarder` custom resource. Before this update, the syntax was `spec.tuning.delivery`. With this update, the syntax is `spec.outputs.<item>.tuning`. (link:https://issues.redhat.com/browse/LOG-5977[LOG-5977])
