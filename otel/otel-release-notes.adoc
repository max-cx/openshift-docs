...
=== Known issues
Curently,  operator cluster-monitoring is disabled (link:https://issues.redhat.com/browse/TRACING-3761[TRACING=3139]). The monitoring can be enabled manually by annotation the operator namespace and creating service monitor.

+
.Workaround
+
Add label to the namespace: `oc label namespace openshift-opentelemetry-operator openshift.io/cluster-monitoring=true`
+
[source,yaml]
----
...
metadata:
  ...
  name: opentelemetry-operator-controller-manager-metrics-monitor
spec:
  endpoints:
