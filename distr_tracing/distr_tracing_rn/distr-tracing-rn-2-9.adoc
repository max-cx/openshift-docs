...
* Missing images to run Operator in restricted network environments (link:https://issues.redhat.com/browse/TRACING-3173[TRACING-3523])

In the current {TempoName} CSV, references to the images of the operand are missing. To workaround this issue you need to specifically provide the Tempo operator related images in the mirroring tool so that the images can be mirrored to the registry.   
