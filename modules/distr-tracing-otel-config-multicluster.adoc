...
= Use the OpenTelemetry Collector to gather observability data from different clusters

To configure a multicluster scenario in OpenShift, you can create one OpenTelemetry
Collector instance in each one of the remote clusters and forward all the telemetry
data to an OpenTelemetry Collector instance 
...
. Create the YAML file to define the `OpenTelemetryCollector` custom resource (CR) in
the edge clusters.
+
