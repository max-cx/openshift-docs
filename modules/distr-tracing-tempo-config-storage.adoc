...
For installation of the Red Hat Elasticsearch Operator, see xref:../logging/cluster-logging-deploying.adoc#cluster-logging-deploying[Installing the {logging-title}].
====
...
----
<1> Enables certificate management by setting `spec.storage.elasticsearch.useCertManagement` to `true` .
====
